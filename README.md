...
Команду запуска возьмите из `runpod_command.txt`. Первый запуск создаёт persistent venv. По умолчанию venv создаётся с `--system-site-packages`, поэтому он видит пакеты из RunPod template. Bootstrap не устанавливает vLLM через pip (`SAFETENSORS_INSTALL_VLLM=0`), чтобы не тянуть новый PyTorch/CUDA/NVIDIA стек.

Используйте template с готовым vLLM/PyTorch/CUDA стеком. Если `vllm` не импортируется, запуск остановится с ошибкой. Только если вы осознанно хотите поставить vLLM через pip, задайте `SAFETENSORS_INSTALL_VLLM=1`; для `torch280` vLLM зафиксирован на версии `0.11.0`, чтобы pip не менял PyTorch/CUDA стек.
